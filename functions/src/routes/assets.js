...
import { getCountryName, formatDuration, formatFileSize, formatDate, formatDateTime } from '../utils/formatting.js';
...
router.get('/', async (req, res, next) => {
  try {
    ...
    const { sort, order } = getSortParams(req.query, ALLOWED_SORT_FIELDS, 'created_at', 'desc');
    ...
    const data = dataResult.rows.map(row => {
      ...
      return {
        ...
        uploader: row.uploader_email ? {
          ...
          name: [row.uploader_first_name, row.uploader_last_name].filter(Boolean).join(' ') || null
        } : null,
        studyProcedure: row.study_procedure_id ? {
          ...
          evaluator: [row.evaluator_first_name, row.evaluator_last_name].filter(Boolean).join(' ') || null
        } : null,
        review: {
          reviewed: row.reviewed || false,
          reviewDate: formatDate(row.review_date),
          reviewer: [row.reviewer_first_name, row.reviewer_last_name].filter(Boolean).join(' ') || null
        },
        comments: row.comments || null
      };
    });
    
    const pagination = buildPaginationResponse(page, limit, total, '/api/v1/assets', req.query);
...
router.post('/query', async (req, res, next) => {
  try {
    ...
    if (trials.length > 0) {
      filters.push(`(acc.trial_name = ANY($${paramIndex}) OR acc.company_name = ANY($${paramIndex}))`);
      params.push(trials);
      paramIndex++;
    }
    ...
    if (dateRange.end) {
      filters.push(`a.created_at <= $${paramIndex++}::date + interval '1 day' - interval '1 second'`);
    ...
    const dataResult = await query(dataQuery, [...params, safeLimit, offset]);
