...
router.get('/', async (req, res, next) => {
  try {
    ...
    const countResult = await query(`SELECT COUNT(*) FROM comments c ${whereClause}`, params);
    ...
    const data = dataResult.rows.map(row => ({
      ...
      author: row.author_id ? {
        id: row.author_id,
        name: [row.author_first_name, row.author_last_name].filter(Boolean).join(' ') || null,
    ...
    const pagination = buildPaginationResponse(page, limit, total, '/api/v1/comments', req.query);
