...
export function getVideoMetrics(records: AssetRecord[]): VideoMetricsData {
  ...
  return {
    ...
    avgSize: formatFileSize(avgSizeBytes),
...
export function getCommentStats(records: AssetRecord[]): CommentStats {
  ...
  return {
    ...
    commentRate: (withComments.length / records.length) * 100,
...
export function filterRecords(records: AssetRecord[], filters: FilterState): AssetRecord[] {
  return records.filter(record => {
    ...
    if (filters.selectedCountries.length > 0 && !filters.selectedCountries.includes(record.siteCountry)) {
      return false;
    }
