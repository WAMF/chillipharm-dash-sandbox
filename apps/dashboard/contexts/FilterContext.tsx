...
export function FilterProvider({ children }: { children: ReactNode }) {
  const [filters, dispatch] = useReducer(filterReducer, defaultFilterState, loadFiltersFromStorage);
  const [presets, setPresets] = useReducer(
    (state: FilterPreset[], action: { type: string; payload?: FilterPreset[] | FilterPreset | string }) => {
      switch (action.type) {
        case 'SET_ALL':
