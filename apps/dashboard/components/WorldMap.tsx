...
import { resolveAllCoordinates, getCountryCoordinates } from '@cp/data-processing';
    ...
    >
      <PopupWithNoSSR>
        <div style={{ textAlign: 'center', padding: '4px' }}>
          <strong style={{ fontSize: '14px', color: '#1f2937' }}>{country}</strong>
          <br />
          <span style={{ fontSize: '18px', fontWeight: 700, color: '#c8102e' }}>
            {count.toLocaleString()}
          </span>
          <br />
          <span style={{ fontSize: '11px', color: '#6b7280' }}>records</span>
        </div>
      </PopupWithNoSSR>
    </MarkerWithNoSSR>
  );
...
export function WorldMap({ countryData, onCountryClick }: WorldMapProps) {
  ...
  useEffect(() => {
    const currentDataString = JSON.stringify(countryData.map(c => c.country));
    if (currentDataString === prevCountryDataRef.current) return;
    prevCountryDataRef.current = currentDataString;
  ...
  const mappedCountries = useMemo(() => {
    ...
    return countryData.filter(c => getCountryCoordinates(c.country) !== null);
    ...
    <div className="flex flex-col gap-4">
      <div className="flex justify-end gap-6">
        <div className="flex flex-col items-center px-4 py-2 bg-neutral-50 rounded-md">
          <span className="text-xl font-bold text-chilli-red">{activeCountries}</span>
          <span className="text-[0.7rem] text-neutral-500 uppercase tracking-wide">Countries</span>
        </div>
        <div className="flex flex-col items-center px-4 py-2 bg-neutral-50 rounded-md">
          <span className="text-xl font-bold text-chilli-red">{totalRecords.toLocaleString()}</span>
          <span className="text-[0.7rem] text-neutral-500 uppercase tracking-wide">Total Records</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 gap-2 max-h-[200px] overflow-y-auto pr-2">
        {countryData.map(({ country, count }) => {
          const isUnmapped = coordinatesResolved && getCountryCoordinates(country) === null;
          return (
            <div
              key={country}
            ...
            >
              <span className="text-xs text-neutral-700 min-w-[100px] flex-shrink-0">
                {country}
                {isUnmapped && (
                  <span className="text-orange-500 font-bold ml-1" title="Location not mapped">*</span>
                )}
              </span>
