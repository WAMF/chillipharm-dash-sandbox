...
export function VideoMetrics() {
    ...
    const {
        ...
        const durationTotal = videoMetrics.durationDistribution.reduce(
            (sum, d) => sum + d.count,
            0
        );
        const sizeTotal = videoMetrics.sizeDistribution.reduce(
            (sum, s) => sum + s.count,
            0
        );
        const commonDuration =
            videoMetrics.durationDistribution.length > 0
                ? videoMetrics.durationDistribution.reduce(
                      (max, d) => (d.count > max.count ? d : max),
                      videoMetrics.durationDistribution[0]
                  ).range
                : 'N/A';
        const commonSize =
            videoMetrics.sizeDistribution.length > 0
                ? videoMetrics.sizeDistribution.reduce(
                      (max, s) => (s.count > max.count ? s : max),
                      videoMetrics.sizeDistribution[0]
                  ).range
                : 'N/A';
        return {
            totalDurationVideos: durationTotal,
            totalSizeVideos: sizeTotal,
            mostCommonDuration: commonDuration,
            mostCommonSize: commonSize,
        };
    }, [videoMetrics]);
    ...
    const handleDurationChartClick = useCallback(
        ...
        <div className="flex flex-col gap-6">
            <div className="mb-2">
                ...
                <p className="text-neutral-500 text-sm">
                    Duration and storage analytics for video assets
                </p>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="bg-white rounded-lg shadow-sm p-6">
                    ...
                    <p className="text-xs text-neutral-500 mb-4">
                        Number of videos by duration range
                    </p>
                    <Chart
                        type="bar"
                    ...
                    <div className="mt-4 overflow-x-auto">
                        <table className="w-full text-sm">
                            ...
                            <tbody>
                                {videoMetrics.durationDistribution.map(
                                    bucket => (
                                        <tr
                                            key={bucket.range}
                ...
                <div className="bg-white rounded-lg shadow-sm p-6">
                    ...
                    <p className="text-xs text-neutral-500 mb-4">
                        Number of videos by file size range
                    </p>
                    <Chart
                        type="bar"
                        data={sizeChartData}
                    ...
                    <div className="flex items-center gap-4 p-4 bg-neutral-50 rounded-lg">
                        ...
                        <div>
                            ...
                            <div className="text-lg font-semibold text-neutral-800">
                                {totalDurationVideos.toLocaleString()} videos
                            </div>
                        </div>
                    </div>
                    <div className="flex items-center gap-4 p-4 bg-neutral-50 rounded-lg">
