...
export function ReviewPerformance() {
  ...
  const turnaroundChartData = useMemo(() => {
    ...
    return {
      ...
      datasets: [{
        label: 'Reviews',
        data: reviewPerformance.turnaroundDistribution.map(t => t.count),
  ...
  const reviewerChartData = useMemo(() => {
    ...
    return {
      ...
      datasets: [{
        ...
        data: reviewPerformance.reviewerStats.map(r => r.reviewCount),
      ...
      datasets: [
        {
          ...
          data: reviewPerformance.reviewTrend.map(t => t.avgTurnaroundDays),
  ...
  const { totalReviewed, sameDayRate } = useMemo(() => {
    ...
    const total = reviewPerformance.turnaroundDistribution.reduce((sum, t) => sum + t.count, 0);
    const sameDayCount = reviewPerformance.turnaroundDistribution.find(t => t.range === 'Same day')?.count || 0;
    const rate = total > 0 ? (sameDayCount / total) * 100 : 0;
    return { totalReviewed: total, sameDayRate: rate };
  }, [reviewPerformance]);

  const handleReviewerClick = useCallback((reviewerName: string) => {
    const reviewerAssets = filteredRecords.filter(r => r.reviewedBy === reviewerName);
  ...
  const handleTurnaroundClick = useCallback((range: string) => {
  ...
  const handleTurnaroundChartClick = useCallback((data: { label: string; index: number; value: number }) => {
  ...
  const handleReviewerChartClick = useCallback((data: { label: string; index: number; value: number }) => {
    if (!reviewPerformance) return;
  ...
  const getPerformanceBadge = useCallback((avgDays: number) => {
    if (avgDays < 1) {
      return { label: 'Excellent', className: 'bg-green-100 text-green-600' };
    }
    if (avgDays < 7) {
      return { label: 'Good', className: 'bg-blue-100 text-blue-600' };
    }
    return { label: 'Needs Attention', className: 'bg-red-100 text-red-600' };
  }, []);
  ...
  if (!reviewPerformance) {
    return (
      <div className="bg-white rounded-lg shadow-sm p-8 text-center">
        <p className="text-neutral-500">No review performance data available.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="mb-2">
        <h2 className="text-xl font-semibold text-neutral-800 mb-1">Review Performance</h2>
        <p className="text-neutral-500 text-sm">Turnaround times, reviewer workload, and review trends</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-lg shadow-sm p-6">
          <h3 className="text-lg font-semibold text-neutral-800 mb-1">Turnaround Distribution</h3>
          <p className="text-xs text-neutral-500 mb-4">Time from upload to review completion</p>
          <Chart
            type="doughnut"
          ...
          <div className="mt-4 flex flex-col gap-2">
            {reviewPerformance.turnaroundDistribution.map((bucket) => (
              <div
                key={bucket.range}
                className="grid grid-cols-[100px_1fr_50px] items-center gap-3 cursor-pointer p-1 -mx-1 rounded hover:bg-neutral-100"
                onClick={() => handleTurnaroundClick(bucket.range)}
                onKeyDown={(e) => e.key === 'Enter' && handleTurnaroundClick(bucket.range)}
                tabIndex={0}
                role="button"
              >
                <span className="text-xs text-neutral-600">{bucket.range}</span>
                ...
                <span className="text-xs text-neutral-500 text-right">{bucket.count}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          <h3 className="text-lg font-semibold text-neutral-800 mb-1">Top Reviewers</h3>
          <p className="text-xs text-neutral-500 mb-4">Review volume by team member</p>
          <Chart
            type="bar"
            ...
            options={{
              ...
              scales: { x: { beginAtZero: true, title: { display: true, text: 'Reviews Completed' } } }
            }}
          />
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        <h3 className="text-lg font-semibold text-neutral-800 mb-1">Review Trend Over Time</h3>
        <p className="text-xs text-neutral-500 mb-4">Monthly turnaround time and review volume</p>
        <Chart
          type="line"
          data={trendChartData}
          height="300px"
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        <h3 className="text-lg font-semibold text-neutral-800 mb-4">Reviewer Performance Details</h3>
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-neutral-50">
                <th className="text-left py-3 px-3 font-semibold text-neutral-600 border-b border-neutral-200">Reviewer</th>
                <th className="text-left py-3 px-3 font-semibold text-neutral-600 border-b border-neutral-200">Reviews</th>
                <th className="text-left py-3 px-3 font-semibold text-neutral-600 border-b border-neutral-200">Avg Turnaround</th>
                <th className="text-left py-3 px-3 font-semibold text-neutral-600 border-b border-neutral-200">Performance</th>
              </tr>
            </thead>
            <tbody>
              {reviewPerformance.reviewerStats.map((reviewer) => {
                const badge = getPerformanceBadge(reviewer.avgTurnaroundDays);
                return (
                  <tr
                    key={reviewer.reviewer}
                    className="cursor-pointer hover:bg-neutral-100"
                    onClick={() => handleReviewerClick(reviewer.reviewer)}
                    onKeyDown={(e) => e.key === 'Enter' && handleReviewerClick(reviewer.reviewer)}
                    tabIndex={0}
                    role="button"
                  >
                    <td className="py-3 px-3 border-b border-neutral-200 font-medium">{reviewer.reviewer}</td>
                    <td className="py-3 px-3 border-b border-neutral-200">{reviewer.reviewCount}</td>
                    <td className="py-3 px-3 border-b border-neutral-200">{reviewer.avgTurnaroundDays.toFixed(1)} days</td>
