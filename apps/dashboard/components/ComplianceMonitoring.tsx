...
export function ComplianceMonitoring() {
  ...
  const getComplianceBadge = useCallback((rate: number) => {
    if (rate >= 90) return { label: 'Excellent', className: 'bg-green-100 text-green-600' };
    if (rate >= 70) return { label: 'Good', className: 'bg-orange-100 text-orange-600' };
    return { label: 'Needs Attention', className: 'bg-red-100 text-red-600' };
  }, []);

  const handleComplianceClick = useCallback((category: string, status: 'compliant' | 'non-compliant') => {
    let matchingAssets;
    let title;
    ...
    if (category === 'Asset Review') {
      ...
      if (status === 'compliant') {
        matchingAssets = filteredRecords.filter(r => r.processed === 'Yes');
        title = 'Processed Assets';
      } else {
        matchingAssets = filteredRecords.filter(r => r.processed === 'No');
  ...
  if (!complianceMetrics || complianceMetrics.length === 0) {
    return (
      <div className="bg-white rounded-lg shadow-sm p-8 text-center">
        <p className="text-neutral-500">No compliance data available.</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="mb-2">
        <h2 className="text-xl font-semibold text-neutral-800 mb-1">Compliance Monitoring</h2>
        <p className="text-neutral-500 text-sm">Regulatory compliance and data quality metrics</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {complianceMetrics.map((metric) => {
          ...
          return (
            <div key={metric.category} className="bg-white rounded-lg shadow-sm p-6 flex flex-col gap-4">
              <div className="flex justify-between items-center">
                <h3 className="text-base font-semibold text-neutral-800">{metric.category}</h3>
              ...
              <div
                ...
                style={{ color: getComplianceColor(metric.complianceRate) }}
              >
              ...
              <div className="flex flex-col gap-2">
                <div
                  ...
                  onClick={() => handleComplianceClick(metric.category, 'compliant')}
                  onKeyDown={(e) => e.key === 'Enter' && handleComplianceClick(metric.category, 'compliant')}
                  tabIndex={0}
                  role="button"
                >
                  <span className="text-neutral-600">Compliant:</span>
                  <span className="font-semibold text-green-600">{metric.compliant}</span>
                </div>
                <div
                  className="flex justify-between text-sm cursor-pointer p-1 -mx-1 rounded hover:bg-neutral-100"
                  onClick={() => handleComplianceClick(metric.category, 'non-compliant')}
                  onKeyDown={(e) => e.key === 'Enter' && handleComplianceClick(metric.category, 'non-compliant')}
                  tabIndex={0}
                  role="button"
                >
                  <span className="text-neutral-600">Non-Compliant:</span>
                  <span className="font-semibold text-red-600">{metric.nonCompliant}</span>
                </div>
                {metric.unknown > 0 && (
                  <div className="flex justify-between text-sm">
                    <span className="text-neutral-600">Unknown:</span>
                    <span className="font-semibold text-neutral-400">{metric.unknown}</span>
                  </div>
                )}
              </div>
              <div className="h-2 bg-neutral-200 rounded overflow-hidden">
                <div
                  ...
                  style={{
                    ...
                    backgroundColor: getComplianceColor(metric.complianceRate)
                  }}
                />
              </div>
            </div>
      ...
      <div className="bg-gradient-to-br from-neutral-50 to-white rounded-lg shadow-sm p-6">
        <h3 className="text-lg font-semibold text-neutral-800 mb-4">Compliance Overview</h3>
