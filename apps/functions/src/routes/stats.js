...
router.get('/libraries', async (req, res, next) => {
    try {
        ...
        const [libraryCount, assetCount, assetsPerLibrary, trialDistribution] =
            await Promise.all([
                query(
                    `SELECT COUNT(*) as total FROM trial_containers tc WHERE tc.type != 'Site' AND tc.deleted_at IS NULL ${trialFilter}`,
                    params
                ...
                query(
                    `
                    SELECT tc.id as library_id, tc.name as library_name, COUNT(a.id) as count
                    FROM trial_containers tc
                    LEFT JOIN assets a ON a.trial_container_id = tc.id AND a.soft_deleted_at IS NULL
                    WHERE tc.type != 'Site' AND tc.deleted_at IS NULL ${trialFilter}
                    GROUP BY tc.id, tc.name
                ...
                query(
                    ...
                    SELECT
                        acc.id as trial_id,
                        COALESCE(acc.trial_name, acc.company_name) as trial_name,
                        COUNT(DISTINCT tc.id) as library_count,
                        COUNT(DISTINCT a.id) as asset_count
                    FROM trial_containers tc
                    JOIN accounts acc ON tc.account_id = acc.id
                    LEFT JOIN assets a ON a.trial_container_id = tc.id AND a.soft_deleted_at IS NULL
                    WHERE tc.type != 'Site' AND tc.deleted_at IS NULL ${trialFilter}
                    GROUP BY acc.id, acc.trial_name, acc.company_name
        ...
        res.json({
            success: true,
            data: {
                totalLibraries: parseInt(libraryCount.rows[0].total) || 0,
                totalAssets: parseInt(assetCount.rows[0].total) || 0,
                assetsPerLibrary: assetsPerLibrary.rows.map(r => ({
                    libraryId: r.library_id,
                    libraryName: r.library_name,
                    count: parseInt(r.count) || 0,
                })),
                trialDistribution: trialDistribution.rows.map(r => ({
                    trialId: r.trial_id,
                    trialName: r.trial_name,
                    libraryCount: parseInt(r.library_count) || 0,
                    assetCount: parseInt(r.asset_count) || 0,
                })),
            },
        });
