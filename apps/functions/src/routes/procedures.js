...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (req.query.trial_id) {
      filters.push(`sp.study_protocol_id IN (SELECT id FROM study_protocols WHERE account_id = $${paramIndex++})`);
    ...
    const data = dataResult.rows.map(row => ({
      ...
      evaluator: row.evaluator_first_name ? {
        name: [row.evaluator_first_name, row.evaluator_last_name].filter(Boolean).join(' ')
      } : null,
      createdAt: formatDateTime(row.created_at),
      updatedAt: formatDateTime(row.updated_at)
    }));
    
    const pagination = buildPaginationResponse(page, limit, total, '/api/v1/procedures', req.query);
